...
class Data_model extends CI_Model {
    ...
    function getdata(){
			 ...
			 $query = $this->db->query('Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END) as quiz_value , SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !="YES" AND b.quiz_value !="No" THEN 1 ELSE 0 END) AS test  From questions a,quiz b Where a.id=b.question_id Group by a.category_id');
	...
	function getAllCategory_yes_no()
		...
		function getAllCategory_department_res($department)
        {
           ...
           $user_type = $this->session->userdata('user_type');
				...
				$org_id = $this->session->userdata('org_id');
		               ...
		               if($user_type==0){
						$query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END) as quiz_value, SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !='YES' AND b.quiz_value !='No' THEN 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND c.department='".$department."' AND a.org_id='".$user_id."' Group by a.category_id");  
						return $query->result();
					  }elseif($user_type==3){
		...
		function getAllCategory_Rating_team($team){
				...
				$org_id = $this->session->userdata('org_id');
		               ...
		               if($user_type==0){
						$query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END)  as quiz_value, SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !='YES' AND b.quiz_value !='No' THEN 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND c.team='".$team."' AND a.org_id='".$user_id."' Group by a.category_id");  
						return $query->result();
					  }elseif($user_type==3){
						$query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END)  as quiz_value, SUM(CASE WHEN b.quiz_status = 1 THEN AND b.quiz_value !='YES' AND b.quiz_value !='No' 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND c.team='".$team."' AND a.org_id='".$org_id."' Group by a.category_id");  
					    return $query->result();
					  }elseif($user_type==2){
						  $query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END)  as quiz_value, SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !='YES' AND b.quiz_value !='No' THEN 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND c.team='".$team."' AND a.org_id='".$org_id."' Group by a.category_id");  
					      return $query->result();
		...
		function getAllCategory_Rating_compere($bar1,$bar2){
				...
				$org_id = $this->session->userdata('org_id');
		               ...
		               if($user_type==0){
						$query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END)  as quiz_value,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 AND (c.team ='".$bar1."' OR c.department='".$bar1."') THEN 1 ELSE 0 END)  as bar1 ,
...
sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 AND (c.team ='".$bar2."' OR c.department='".$bar2."') THEN 1 ELSE 0 END)  as bar2, SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !='YES' AND b.quiz_value !='No' THEN 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND (c.team ='".$bar1."' OR c.department='".$bar1."') AND (c.team ='".$bar2."' OR c.department='".$bar2."') AND a.org_id='".$org_id."' AND a.category_id!='' Group by a.category_id");  
						...
						$query = $this->db->query("Select a.id, a.category_id ,sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 THEN 1 ELSE 0 END)  as quiz_value,COALESCE(sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 AND (c.team ='".$bar1."' OR c.department='".$bar1."') THEN 1 ELSE 0 END),0)  as bar1 ,
COALESCE(sum(b.quiz_value) / SUM(CASE WHEN b.quiz_status = 1 AND (c.team ='".$bar2."' OR c.department='".$bar2."') THEN 1 ELSE 0 END),0)  as bar2, SUM(CASE WHEN b.quiz_status = 1 AND b.quiz_value !='YES' AND b.quiz_value !='No' THEN 1 ELSE 0 END) AS test From questions a,quiz b,organization c Where a.id=b.question_id AND b.emp_id=c.id AND (c.team ='".$bar1."' OR c.department='".$bar1."') AND (c.team ='".$bar2."' OR c.department='".$bar2."') AND a.category_id!='' Group by a.category_id");  
						return $query->result();
					  }                       

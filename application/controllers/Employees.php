...
class Employees extends CI_Controller {
	function Employees(){
	 ...
	 function index(){
		...
		$data["fetch_data"] = $this->employees_model->employees_fetch_data();  
		$data['organization'] = $this->employees_model->getAllorganization();
	...
	function getTeam(){
		$keyword=$this->input->post('keyword');
        $data=$this->employees_model->GetRowTeam($keyword);        
        echo json_encode($data);
	}
	...
	public function update_data(){  
           ...
           $data["user_data"] = $this->employees_model->fetch_single_data($user_id);  
           $data["fetch_data"] = $this->employees_model->employees_fetch_data();  
		   $this->load->view('header');
           $this->load->view("employees", $data);  
		   $this->load->view('footer');
	            ...
	            while(($line = fgetcsv($csvFile)) !== FALSE){
	             ...
	             $this->session->set_flashdata('update','Email already exist. Please try another Email or CSV file' );
	               ...
	               // $this->session->set_flashdata('SUSS', 'SUCCESS : This email '. $to_email .' has been added. ');
	                    ...
	                    $this->db->insert("organization", array("name"=>$line[0], "email"=>trim($line[1]), "department"=>$line[2], "designation"=>$line[3],"login_type"=>2,"password"=>$tempass, "mobile_number"=>$line[4],"org_id"=>$org_id));
